...
\label{sec-3-3-3}
    ...
    system. This library is also used to parse the parameter file into the
    system, and as such defines the syntax that the parameter file must
    follow. We attempted to find a library equivalent to the Java Properties
    class, which allows the structured reading and storage of parameters. We
    were, however, unable to find such a library and therefore had to implement
    our own version. The syntax is very simple; the \texttt{\#} symbol defines a
    comment, and that line will be ignored when parsing. A parameter is defined
    as an string of ASCII characters followed by a single space, followed by
    more ASCII characters. This library also reads in event data files, which are
...
\begin{itemize}
\item explanation of the various bits to do with stuttering and so on
\item How data is parsed in and stuff
\item how the experiment numbers are incremented
\end{itemize}
\subsection{Launcher}
\label{sec-3-5}

\begin{itemize}
\item what thing it uses getopt
\item user-friendliness (help output)
\item struct usage to improve ease of modification
\end{itemize}
\subsection{Scripts}
\label{sec-3-6}
...
\label{sec-3-7}
   ...
   Usage instructions for the system can be found in Appendix \ref{sec-11}
\section{Development}
\label{sec-4}
