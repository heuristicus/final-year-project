...
INDIR=/media/michal/Edison/fyp/new/random/rand_params
STREAM_DIR=/media/michal/Edison/fyp/new/random/rand_streams
GOODNESS_LOC=/media/michal/Edison/fyp/new/random/
LAUNCHER_LOC=/home/michal/Dropbox/study/university/final_year/final-year-project/src/deltastream
OUTDIR=/media/michal/Edison/fyp/new/random/rand_time_delay
...
for TD_EST_TYPE in pmf area; do
	...
	for FUNC in ${FUNC_VALUES[@]}; do
	    if [ ! -d "$OUTDIR/$TD_EST_TYPE/$FOLDER_PREFIX$ALPHA" ]; then
		mkdir $OUTDIR/$TD_EST_TYPE/$FOLDER_PREFIX$ALPHA/$FUNC_PREFIX$FUNC
	    fi	    
	    for TYPE in ${ENAMES[@]}; do
		if [ ! -d "$OUTDIR/$TD_EST_TYPE/$FOLDER_PREFIX$ALPHA/$FUNC_PREFIX$FUNC" ]; then
		    mkdir $OUTDIR/$TD_EST_TYPE/$FOLDER_PREFIX$ALPHA/$FUNC_PREFIX$FUNC/$TYPE
		fi

		echo "alpha=$ALPHA, func=$FUNC, type=$TYPE" >> $BESTFILE
